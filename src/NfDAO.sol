...
contract CS2 is IERC20, Ownable,DSTest {
    ...
    uint256 public _buyLiquidityFee = 900; //9%
    ...
    address public _token ;//= 0x55d398326f99059fF775485246999027B3197955;//usdt
    ...
    constructor(address usdt,address router) {
        address admin = msg.sender;
        _swapTokensAtAmount = _tTotal.mul(1).div(10 ** 4);
        _tOwned[admin] = _tTotal;// 100 亿
        ...
        _token.call(
            abi.encodeWithSelector(0x095ea7b3, _uniswapV2Router, ~uint256(0))//approve
        );
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        console2.log(" _transfer amount",amount);
         emit log_named_decimal_uint("_transfer amount",amount, 18);
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (_inviterFee > 0) {
               ...
               // console2.log("inviterFee",inFee);
                emit log_named_decimal_uint("inviterFee",inFee, 18);
            }
            if (getBuyFee() > 0 && from == _uniswapV2Pair) {
                ...
                amount = takeBuy(from, amount);
                 console2.log("takeBuy",amount);
                 emit log_named_decimal_uint("takeBuy",amount, 18);
            } else if (getSellFee() > 0 && to == _uniswapV2Pair) {
                //sell
                require(amount <= _maxSellTax, "Transfer limit");
                amount = takeSell(from, amount);
                console2.log("SellFee",amount);
                 emit log_named_decimal_uint("SellFee",amount, 18);
                ...
                if (_tranFee == 1) amount = takeBuy(from, amount);
                 ...
                 console2.log("_tranFee",amount);
                 emit log_named_decimal_uint("_tranFee",amount, 18);
            }
        ...
        console2.log("_basicTransfer",amount);
    }
    ...
    function takeBuy(
        ...
        uint getBuyFee2 = getBuyFee();
        console2.log("takeBuy getBuyFee ",getBuyFee2);
        uint256 fees = amount.mul(getBuyFee()).div(10000);
        console2.log("takeBuy fees ",fees);
        if(amount >= 100000e18) {
           fees = amount.mul(800).div(10000); 
        }
        console2.log("takeBuy2 fees ",fees);
        console2.log("_buyBurnFee",amount.mul(_buyBurnFee).div(10000));
        _basicTransfer(
            from,
            address(this),
            fees.sub(amount.mul(_buyBurnFee).div(10000))// 这里存在减法溢出  这里是不是要写_buyLiquidityFee
        );
        if (_buyBurnFee > 0) {
            _basicTransfer(
                from,
    ...
    function takeSell(
        ...
        if(amount >= _balance) {
           amount = amount.mul(9900).div(10000);//如果全部转的化 只能转这些
        }
        uint getSellFee2 = getSellFee();
        console2.log("takeSell getSellFee2 ",getSellFee2);
        uint256 fees = amount.mul(getSellFee()).div(10000);//SellFee  10%
        _basicTransfer(
            from,
            _marketAddr,
        ...
        if((_tTotal.sub(burnEndAmount)) > burnTotal) {
            ...
            console2.log("burnTotal ",burnTotal);
        }
        _amount = amount.sub(fees);
        if((_tTotal.sub(burnEndAmount)) <= burnTotal) {
    ...
    function getPar(address account) public view returns (address par) {
       // console.log("par is",par);
        ...
        if (par == address(0)) {
            ...
            console.log("par is",_inviter[account]);
            par = _inviter[account];
            console.log("par2 is",_inviter[account]);
        }
        //console.log("par2 is",par);
    }

    function setInviKillBlock(uint value) public {// 这个需求权限吗
