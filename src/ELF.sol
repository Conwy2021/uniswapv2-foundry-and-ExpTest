...
contract Token is Context, IERC20, Ownable ,Test{
    ...
    constructor (
        ...
        _minimumTokensBeforeSwap = 1 * 10**_decimals;//交换前的最低代币数量
        marketingWalletAddress = payable(marketingAddress);
        teamWalletAddress = payable(teamAddress);
        _totalTaxIfBuying = _buyLiquidityFee.add(_buyMarketingFee).add(_buyTeamFee);
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellTeamFee);
        _totalDistributionShares = _liquidityShare.add(_marketingShare).add(_teamShare);//总分配股份
        uniswapV2Router = _uniswapV2Router;
        _allowances[address(this)][address(uniswapV2Router)] = _totalSupply;//给router 授权此合约所有代币权限  这里是不是有问题啊  conwy
        isExcludedFromFee[owner] = true;
        isExcludedFromFee[address(this)] = true;

        isWalletLimitExempt[owner] = true;//钱包限额是否豁免
        isWalletLimitExempt[address(uniswapPair)] = true;
        isWalletLimitExempt[address(this)] = true;
        isWalletLimitExempt[deadAddress] = true;

        isTxLimitExempt[owner] = true;//这是交易限制
        isTxLimitExempt[deadAddress] = true;
        isTxLimitExempt[address(this)] = true;

        isMarketPair[address(uniswapPair)] = true;//是市场排
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(sender == uniswapPair && block.number < first + kill){
            return _basicTransfer(sender, receiveAddress, amount);//普通转账  这里不会进来
        }
        if(inSwapAndLiquify)//inSwapAndLiquify false
        {
            return _basicTransfer(sender, recipient, amount);//普通转账 不会进来
        }
        else
        {
            if(!isTxLimitExempt[sender] && !isTxLimitExempt[recipient]) {//是否交易限制
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 tokensForLP = tAmount.mul(_liquidityShare).div(_totalDistributionShares).div(2);//tAmount x2/9/2
        uint256 tokensForSwap = tAmount.sub(tokensForLP);// 8/9

        swapTokensForEth(tokensForSwap);// 8/9 转为 ETH
        uint256 amountReceived = address(this).balance;// 查询合约有多少ETH

        uint256 totalBNBFee = _totalDistributionShares.sub(_liquidityShare.div(2));// 10

        uint256 amountBNBLiquidity = amountReceived.mul(_liquidityShare).div(totalBNBFee).div(2);//amountReceivedx2/8/2   8分之一 eth
        uint256 amountBNBTeam = amountReceived.mul(_teamShare).div(totalBNBFee);// 0  
        uint256 amountBNBMarketing = amountReceived.sub(amountBNBLiquidity).sub(amountBNBTeam);// 剩下的 8分之3

        if(amountBNBMarketing > 0)
            transferToAddressETH(marketingWalletAddress, amountBNBMarketing);//剩下的 8分之3

        if(amountBNBTeam > 0)
            transferToAddressETH(teamWalletAddress, amountBNBTeam);//8分之4 

        if(amountBNBLiquidity > 0 && tokensForLP > 0)//9分之一的token 和 8分之1的eth 去添加流动性
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {// from 是 pair 购买 或者移除流动性
            feeAmount = amount.mul(_totalTaxIfBuying.sub(_buyDestroyFee)).div(100);//9%
            if(_buyDestroyFee > 0 && _tFeeTotal < _maxDestroyAmount) {//_buyDestroyFee 为0 不进入
                destAmount = amount.mul(_buyDestroyFee).div(100);
                destroyFee(sender,destAmount);
            }
        }
        else if(isMarketPair[recipient]) {//to 是pair 出售或者是添加流动性 
            feeAmount = amount.mul(_totalTaxIfSelling.sub(_sellDestroyFee)).div(100);//9%
            if(_sellDestroyFee > 0 && _tFeeTotal < _maxDestroyAmount) {////_sellDestroyFee 为0 不进入
        ...
        if(feeAmount > 0) {
            _balances[address(this)] = _balances[address(this)].add(feeAmount);//收取的手续费 留到此合约的
