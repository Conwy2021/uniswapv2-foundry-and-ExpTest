...
///@notice 已弃用。请改用 InterestRateModelV2。
///动态利率模型实现
///@dev 模型存储一些筒仓特定数据。如果更换模型，重新部署后需要设置正确的配置
///无缝服务。设计细节请参阅有关模型的单独简报。
///@custom:security-contact security@silo.finance
///
///已弃用。请改用 InterestRateModelV2。
///
import "forge-std/console.sol";
import "forge-std/Test.sol";// forge test --mp ./src/interestRateModel/InterestRateModel.t.sol -vvv
contract InterestRateModel is IInterestRateModel, TwoStepOwnable,Test {
    ...
    function calculateCurrentInterestRate(
        ...
        uint256 _blockTimestamp
    ) public  override returns (uint256 rcur) {
    ...
    function calculateCompoundInterestRate(
        ...
        (rcomp, ri, Tcrit,) = calculateCompoundInterestRateWithOverflowDetection(//通过溢出检测计算复利
            _c,
            _totalDeposits,
            _totalBorrowAmount,
            _interestRateTimestamp,
    ...
    function calculateCompoundInterestRateWithOverflowDetection(
        ...
        _l.u = EasyMath.calculateUtilization(DP, _totalDeposits, _totalBorrowAmount).toInt256();//10**13 资金利用率
        
       emit log_named_decimal_int("_l.u", _l.u, 18);
        // slopei := ki * (u0 - uopt )
        _l.slopei = _c.ki * (_l.u - _c.uopt) / _DP;// 使用率差值*ki （10000000000000.000000000000000000-9000000000000000000)*k
       
        emit log_named_decimal_int("_c.ki",_c.ki, 18);
        emit log_named_decimal_int("_l.slopei",_l.slopei, 18);

        if (_l.u > _c.ucrit) {//走这里 资金利用率大于90%
            // rp := kcrit * (1 + Tcrit) * (u0 - ucrit ) 951293759513*(1+0)(10**13-0.9)
            _l.rp = _c.kcrit * (_DP + Tcrit) / _DP * (_l.u - _c.ucrit) / _DP;
              emit log_named_decimal_int("_l.rp",_l.rp, 18);//9512937.595129143835616438
            // slope := slopei + kcrit * beta * (u0 - ucrit ) 3.67+0.000...00951293759513*0.00..0069444444444444*(10**13-0.9)
            _l.slope = _l.slopei + _c.kcrit * _c.beta / _DP * (_l.u - _c.ucrit) / _DP;
              emit log_named_decimal_int(" _l.slope", _l.slope, 18);
            // Tcrit := Tcrit + beta * T
            Tcrit = Tcrit + _c.beta * _l.T;//0+69444444444444*(39-15)
            emit log_named_decimal_int("Tcrit", Tcrit, 18);
        } else {
            // rp := min (0, klow * (u0 - ulow ))
        ...
        // rlin := klin * u0 # lower bound between t0 and t1
        _l.rlin = _c.klin * _l.u / _DP; // 396372400*10**13
        emit log_named_decimal_int("_l.rlin", _l.rlin, 18);
        // ri := max(ri , rlin )
        ri = _max(ri , _l.rlin);//3963.724000000000000000
        emit log_named_decimal_int("ri", ri, 18);
        // r0 := ri + rp # interest rate at t0 ignoring lower bound
        _l.r0 = ri + _l.rp;//3963.724000000000000000+9512937.595129143835616438
         emit log_named_decimal_int("_l.r0",_l.r0, 18);
        // r1 := r0 + slope *T # what interest rate would be at t1 ignoring lower bound
        _l.r1 = _l.r0 + _l.slope * _l.T;//9516901.319129143835616438+ 664.290769999940250531*(39-15)
          emit log_named_decimal_int(" _l.r1", _l.r1, 18);
        ...
        if (_l.r0 >= _l.rlin && _l.r1 >= _l.rlin) {
            ...
            emit log_named_decimal_int(" _l.x1", _l.x, 18);  // 228596947.400859434846947440
        } else if (_l.r0 < _l.rlin && _l.r1 < _l.rlin) {
            // lower bound is active during the whole time
            // rcomp := exp( rlin * T) - 1
            _l.x = _l.rlin * _l.T;
             emit log_named_decimal_int(" _l.x", _l.x, 18);
        } else if (_l.r0 >= _l.rlin && _l.r1 < _l.rlin) {
        ...
        ri = _max(ri + _l.slopei * _l.T, _l.rlin);
         emit log_named_decimal_int(" ri", ri, 18);
        //检查下面是否有溢出。如果发生溢出，ri 和 Tcrit 将被设置回零。 Rcomp 是
        //计算不会使totalBorrowedAmount、totalDeposits 溢出。
