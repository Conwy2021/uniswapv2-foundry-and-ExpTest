...
contract CS2 is Context, IERC20, Ownable ,Switch{
    ...
    address public csFeePlusAddress = address(0xF647Ea703303a0173B8F24Ec102A6540Ff038f2F);
    address public csFundAddress = address(0xF647Ea703303a0173B8F24Ec102A6540Ff038f2F);
    ...
    function _isLiquidity(address from ,address to)internal view  returns(bool isAdd){
        ...
        isAdd = bal > r && bal - r > addPriceTokenAmount && exPairs[to] && from!=to;  
    }
    ...
    function _transfer(
        ...
        bool takeBuyFee = isBuyFee && !feeWhiteList[to] &&  exPairs[from] && !_isLiquidity(from,to);
         console2.log("takeTransFee",takeTransFee);
         console2.log("takeSellFee",takeSellFee);
         console2.log("takeBuyFee",takeBuyFee);
        if (exPairs[to] && _isLiquidity(from,to)){
        ...
        if (takeBuyFee){
            require(amount <= limitBuy,"Buying too much ");
        ...
        if(takeSellFee){
            ...
            require(amount <= limitSell,"Sell too much");
    ...
    function _getBuyParam(uint256 tAmount,FeeParam memory param) private   {
        if (totalBurnAmount + (tAmount * totalBuyFee / 1000) > maxBurnAmount){//maxBurnAmount 90
            param.tBuyFee = maxBurnAmount - totalBurnAmount;// 这里其实也有问题  total 100
        }else{
            param.tBuyFee = tAmount * totalBuyFee / 1000;
        }
        totalBurnAmount += param.tBuyFee; //90
    ...
    function _getSellParam(uint256 tAmount,FeeParam memory param) private  returns( FeeParam memory)  {
        ...
        param.tSellFee = tAmount.mul(totalSellFee).div(1000,"sellfee error"); 
        param.tSellPlusFee = tAmount.mul(plusFee).div(1000,"sellplusfee error");
